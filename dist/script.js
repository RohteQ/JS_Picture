...
const accordion = triggersSelector => {
  ...
  btns.forEach(btn => {
    btn.addEventListener('click', function () {
      // Скрываем все блоки и убираем классы активности
      hideBlock();
  
      // Активируем текущий блок, если он не был активен
...
const burger = (menuSelector, burgerSelector) => {
  ...
  document.addEventListener('click', (e) => {
    if (menuElem.style.display == 'block' && !menuElem.contains(e.target) && !burgerElem.contains(e.target)) {
      menuElem.style.display = 'none';
    }
  });
  const listItems = menuElem.querySelectorAll('li');
  listItems.forEach(item => {
    item.addEventListener('click', (e) => {
      // Проверка, что клик был по элементу <li> или его потомкам
